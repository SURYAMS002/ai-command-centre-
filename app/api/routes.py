    ...
    try:
        ...
        try:
            ...
            if target_pump_command != current_pump_status:
                ...
                if log_action:
                    cursor.execute(
                        "INSERT INTO operation_logs (timestamp, farmer_command, action, details, status) VALUES (?, 'ESP32 Telemetry Auto-Control', ?, ?, 'SUCCESS');",
                        (now, log_action, decision_reasoning)
                    )
                conn.commit()
